...
# GraphQLのqueryとmutation

GraphQLではqueryでデータの取得、mutationでデータの更新を行います。バックエンドのリゾルバーを作成した時に`@Query`がついている関数がqueryで
呼び出せます。`@Mutation`がついている関数がmutationで呼び出せる関数になります。
...
使用され、バックエンドに用意した名称とパラメータを使ってデータの更新を行います。
